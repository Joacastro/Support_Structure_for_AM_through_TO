...
f0val,df0dx,fval,dfdx,low,upp,a0,a,c,d,set_mma);
%
...
albefa = 0.1;
%if iter <=2
asyinit = 0.5;%0.2;%0.5 (origine) - 0.1(fonction (works with pmean)~)
asyincr = 1.2;%1.05; % 1.2 (origine)- 1.05(fonction (works with pmean) ~)
asydecr = 0.7;%0.4; %0.7 (origine)- 0.5(fonction (works with pmean) ~)
asyincr = set_mma*asyincr;
asydecr = set_mma*asydecr;
eeen = ones(n,1);
eeem = ones(m,1);
zeron = zeros(n,1);
