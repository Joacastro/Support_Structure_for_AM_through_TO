function [g,dgdz,MS] = MaxSize_MinGap(h,dhdy,g,dgdz,DI,MS,penal,nx,ny,Iv)
...
if ~isfield(MS,'DII')
    ...
    [MS.DII,MS.cv,MS.epsi] = MSregions(ECtrd,Rmin,Rmax,nx*ny,ny,MS.epsi); %  DII matrix
end 
%% ===================== Maximum Size Constraint using p-mean ==================
p  = MS.p; epsi = MS.epsi;                      % p-exponent & epsilon
q = min(2.5-penal,MS.qMax);                        % penalization of voids   
...
for el = 1:NElem
    ...
    epsi(el,1) = (EPSI(1)-EPSI(2))/(nely-1)*(yi-1)+EPSI(2);
    Rin = 0;
    %---------------------------------
